 ...
 * \return Si se pudo realizar devuelve un 0, como se�al de OK, sino devuelve -1
 *
 */
 ...
 * \param cantidad maxima de elementos
 * \return Si se pudo realizar devuelve un 0, como se�al de OK y se imprime los elementos por pantalla, sino devuelve -1
 *
 */
 ...
 * \return devuelve los elementos que estan en el array de enteros,con un 0 en se�al de OK, sino devuelve -1 en se�al de error
 *
 */
 ...
 * \return devuelve la posicion y/o el valor del maximo del array,con un 0 en se�al de OK, sino devuelve -1 en se�al de error
 *
 */
 ...
 * \param cantidad maxima de elementos del array de enteros
 * \param direccion de memoria de la variable a la que se le asignara el valor obtenido
 * \return devuelve la posicion y/o el valor del minimo del array,con un 0 en se�al de OK, sino devuelve -1 en se�al de error
 *
 */
 ...
 * \param array del cual se obtendran los elementos
 * \param cantidad maxima de elementos del array de enteros
 * \return devuelve un 0 en se�al de que realizo el ordenamiento de elementos
 *
 */
 ...
 * \return devuelve el string que se cargo, con un 0 en se�al de que salio todo OK, sino devuelve -1
 *
 */
 ...
 *
 */
int imprimirArrayString(char aNombres[][CARACTERES], int cantidad)
{
	int i;
	int retorno = -1;
 ...
 *
 */
int ordenarArrayString(char aNombres[][CARACTERES], int cantidad){
	int i;
	int retorno = -1;
	...
	if(aNombres != NULL && cantidad>0)
	{
		...
		do
		{
			...
			{
				if(strncmp(aNombres[i],aNombres[i+1],CARACTERES)>0)
				{
					fSwap = 1;
					strncpy(buffer,aNombres[i],CARACTERES);
					strncpy(aNombres[i],aNombres[i+1],CARACTERES);
					strncpy(aNombres[i+1],buffer,CARACTERES);
				}
			}
		}while(fSwap);
	}
 ...
 */
 ...
 int menu( char opciones[]  ){
 	...
 	opcion=getInt("Ingrese la opcion deseada","Error",0,100);
